...
class SoleConnectionAsserts
{
    ...
    {
        assertThat(open.getProperties(), is(notNullValue()));
        assertThat(open.getProperties(), hasKey(Symbols.AMQP_CONN_ESTABLISHMENT_FAILED));
        assertThat(open.getProperties(), hasEntry(Symbols.AMQP_CONN_ESTABLISHMENT_FAILED, true));
    }

    static void assumeConnectionEstablishmentFailed(final Open open)
    {
        assumeTrue(is(notNullValue()).matches(open.getProperties()));
        assumeTrue(hasKey(Symbols.AMQP_CONN_ESTABLISHMENT_FAILED).matches(open.getProperties()));
        assertThat(open.getProperties(), hasEntry(Symbols.AMQP_CONN_ESTABLISHMENT_FAILED, true));
    }
    ...
    static void assertResourceLocked(final Close close)
    {
        ...
        assertThat(close.getError().getInfo(), is(equalTo(Map.of(Symbols.SOLE_CONNECTION_ENFORCEMENT, true))));
    }
    ...
    static void assertInvalidContainerId(final Close close)
    {
        ...
        assertThat(close.getError().getInfo(), is(equalTo(Map.of(Symbols.INVALID_FIELD, Symbols.CONTAINER_ID))));
    }
}
