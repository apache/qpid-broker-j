...
public class MinExpression<T, R> extends AbstractAggregationExpression<T, T>
{
    /**
     ...
     */
    public MinExpression(final String alias, final List<ExpressionNode<T, ?>> args)
    {
        super(alias, args);
    ...
    public T aggregate(final List<T> items)
    {
        ...
        final Optional<Comparable<R>> optResult = args.stream().map(result -> (Comparable<R>) result).min(comparator);
