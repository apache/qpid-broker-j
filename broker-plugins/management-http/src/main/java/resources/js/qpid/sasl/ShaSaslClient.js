                             ...
                             _getResponse:       function(data)
                                                 {
                                                    if (this._state == "initial")
                                                    {
                                                      ...
                                                      {
                                                        this._state = "error";
                                                        throw {message: "Username '" + challenge.username +
                                                                        "' is invalid"};
                                                      }
                                                    }
                                                    else if (this._state == "initiated")
                                                    {
                                                        ...
                                                        {
                                                          this._state = "error";
                                                          throw {message: "Authentication error - server nonce does " +
                                                                          "not start with client nonce"};
                                                        }
                                                        else
                                                        {
                                                            var salt = CryptoJS.enc.Base64.parse(parts[1].substring(2));
                                                            var iterationCount = parts[2].substring(2);
                                                            var saltedPassword = generateSaltedPassword(digest, salt,
                                                                                        this._password, iterationCount);
                                                            var clientFinalMessageWithoutProof = "c=" +
                                                                                  toBase64(gs2_header) + ",r=" + nonce;
                                                            var authMessage = this._clientFirstMessageBare + "," +
                                                            ...
                                                            var response = toBase64(clientFinalMessageWithoutProof +
                                                    ...
                                                    else if (this._state == "generated")
                                                    {
                                                        ...
                                                        if (this._serverSignature.toString(CryptoJS.enc.Base64) ==
