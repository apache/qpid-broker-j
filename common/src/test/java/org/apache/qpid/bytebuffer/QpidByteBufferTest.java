...
public class QpidByteBufferTest extends QpidTestCase
{
    public void testPooledBufferIsZeroedLoan() throws Exception
    {
        ...
        QpidByteBuffer.createPool(bufferSize, 1);
        QpidByteBuffer buffer = QpidByteBuffer.allocateDirectFromPool(bufferSize);

        buffer.put((byte) 0xFF);
        buffer.dispose();

        buffer = QpidByteBuffer.allocateDirectFromPool(bufferSize);
        buffer.limit(1);
